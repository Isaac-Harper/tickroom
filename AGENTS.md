...
MEASURED ON THIS TREE, not estimated: `npx vitest run` collects 1142 tests
...
`TICKROOM_TEST_REDIS_URL=redis://127.0.0.1:6499` it is 1079 passed and 63
skipped across 32 files run and 12 skipped, still exit 0, because
`tests/memory.test.ts` needs nothing and runs either way); the same suite was
1130 across 43 files at 0.2.0, before `src/testing/lockstep.test.ts` and the
relay's fragment cases; `npx tsc --noEmit`
...
these SUM to the total rather than approximating it: core 208, server 364,
client 274, codec 109, adapters 69, testing 8, examples 46, `tests/` 64.
...
146, `relay.test.ts` 127, `connection.test.ts` 122, `interpolation.test.ts` 48,
